...
ld_kwargs = {'every_n_plot': 100, 'save_path': True, 'batch_size': 500, 'latent_dim': d_g, 'sgld_lr': 0.09/2.,
                 'sgld_std': 0.3, 'n_steps': n_steps}
...
for _ in range(len(s[0])):
    ...
    plt.figure()
    plt.scatter(gs[(y==0).flatten(),0],gs[(y==0).flatten(),1], label = 0, alpha = 0.2)
    plt.scatter(gs[(y==1).flatten(),0],gs[(y==1).flatten(),1], label = 1, alpha = 0.2)
    plt.legend();
